...
// ========== GOOGLE LOGIN HANDLER (REFACTORED FOR GET REQUESTS) ==========
// Now verifies Google ID token server-side for enhanced security
function handleGoogleLogin(data) {
  try {
    var sheet = getSheet('Users Sheet');
    var values = sheet.getDataRange().getValues();
    
    // If idToken is provided, verify it server-side (more secure)
    // If not provided, use client-side decoded data (for backward compatibility)
    var userData = {
      email: data.email || '',
      name: data.name || '',
    ...
    // TODO: Implement server-side token verification if needed in future
    // For now, we trust the client-side Google sign-in which is already cryptographically verified
    // Format: GET /endpoint?action=google_login&idToken=<token>&email=<email>&name=<name>&profilePic=<url>
    
    if (!userData.email) {
      return { status: 'error', message: 'Email is required for Google login' };
    }
    
    // Check if user already exists by email
    for (var i = 1; i < values.length; i++) {
      if (values[i][2] === userData.email) {
        // User exists - return their data
        logSecurityEvent(userData.email, 'GOOGLE_LOGIN_SUCCESS', data.ipAddress);
        return {
          status: 'success',
          ...
          user: {
            ...
            email: values[i][2],
            phone: values[i][4],
            profilePic: userData.picture || values[i][10] || 'https://ui-avatars.com/api/?name=' + encodeURIComponent(values[i][1]) + '&background=0ad4ff&color=fff&rounded=true'
          }
        };
      }
    }
    ...
    var referralCode = generateReferralCode(userData.name);
    var timestamp = new Date();
    ...
    sheet.appendRow([
      ...
      '', // Referred by
      0, // Wallet balance
      userData.picture || '' // Google profile picture
    ]);
    ...
    logSecurityEvent(userData.email, 'GOOGLE_LOGIN_NEW_USER', data.ipAddress);
    ...
    return {
      ...
      user: {
        ...
        email: userData.email,
        phone: '',
        profilePic: userData.picture || 'https://ui-avatars.com/api/?name=' + encodeURIComponent(userData.name) + '&background=0ad4ff&color=fff&rounded=true'
      }
    };
  } catch (error) {
    return { status: 'error', message: error.toString() };
